...
impl AnalyticsEngine {
    pub fn new(
        ...
        config: AppConfig,
        block_tx: mpsc::Sender<(String, u64)>,
    ) -> Self {
        Self {
            states,
            config,
    ...
    pub fn process_entry(&mut self, entry: AccessLogEntry) -> Option<(String, u64)> {
        ...
        match entry.request_type {
            ...
            RequestType::Api => {
                ...
                *state
                    .custom_counters
                    .entry("api_requests".to_string())
                    .or_insert(0) += 1;
            }
            RequestType::Page | RequestType::WpAdmin => {
                state.page_window.add_request(entry.timestamp)
            }
            RequestType::Static => {
                *state
                    .custom_counters
                    .entry("static_requests".to_string())
                    .or_insert(0) += 1;
            }
            _ => {}
        }
        ...
        if entry.request_type == RequestType::WpLogin && entry.method == "POST" {
            *state
                .custom_counters
                .entry("login_attempts".to_string())
                .or_insert(0) += 1;
        }
        if entry.status == 404 {
            *state
                .custom_counters
                .entry("404_errors".to_string())
                .or_insert(0) += 1;
        }
        // Eventos específicos
