...
async fn api_ips(State(state): State<AppState>) -> Json<Vec<serde_json::Value>> {
    let analytics = state.analytics.read();
    let mut ips: Vec<serde_json::Value> = analytics.states.iter()
        .map(|(ip, s)| serde_json::json!({
            "ip": ip.to_string(),
            "vhost": s.vhost,
        ...
        .collect();
    ips.sort_by(|a, b| b["score"].as_f64().partial_cmp(&a["score"].as_f64()).unwrap_or(std::cmp::Ordering::Equal));
    Json(ips)
}
...
async fn api_unblock(State(state): State<AppState>, Json(payload): Json<IpActionPayload>) -> Json<serde_json::Value> {
    {
        let mut enforcer = state.enforcer.write();
